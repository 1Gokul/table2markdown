$(document).ready(function () {
  ...
  $(".custom-file-input").on("change", function () {
    ...
    var extension = $(".custom-file-input")
      .val()
      .split("/")
      .pop()
      .split(".")[1];
    ...
    if ($.inArray(extension, validImageTypes) < 0) {
      ...
      if ($("#csvFile").prop("files")[0].size > 10 * 1024 * 1024) {
        send_alert(
          "This file is larger than the 10MB limit! Please select a smaller one.",
          "danger"
        );
        $("#submit-button").prop("disabled", true);
      } else {
        if ($(".alert").length)
          send_alert("Nice, this one's good to go!", "primary");
  ...
  $("#submit-button").on("click", function () {
    ...
    var convertOption = $(
      "input[name=modify-or-not]:checked",
      "#csv-form"
    ).val();
    if (convertOption) {
      ...
      $(this)
        .prop("disabled", true)
        .html('Converting...<i class="fas fa-circle-notch fa-spin ml-2"></i>');
