$(document).ready(function () {
  ...
  function deselect_cell() {
    ...
    selectedCellY = 0;
    $(".keyboard-nav > div")
      .not(".movement-key, .undo-key, .redo-key")
      .addClass("grey-out");
  }
  ...
  function move_selected_cell(direction) {
    ...
    console.log(
      "You have gone to: [" + selectedCellY + "][" + selectedCellX + "]"
    );
  ...
  function display_result_table(responseObject) {
    ...
    html +=
      ...
      '"target="_blank" class="btn submit-download-link">Raw<br>(Press <kbd>I</kbd>)</a>';
    html +=
      '<a href="' +
      Flask.url_for("get_table", {
        view_type: "download",
      ...
      '"target="_blank" class="btn submit-download-link">Download<br>(Press <kbd>O</kbd>)</a></div></div></div>';
  ...
  function generate_table() {
    ...
    $(".generate-table-button > button").html(
      'Converting...<i class="fas fa-circle-notch fa-spin ml-2"></i>'
    );
  ...
  function undo_action() {
    ...
    if (historyPosition > 0) {
      ...
      $("#input-table")
        .html("")
        .append(actionHistory[historyPosition - 1]);
